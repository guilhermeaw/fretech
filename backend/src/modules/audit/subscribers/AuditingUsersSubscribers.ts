...
@EventSubscriber()
export class AuditingUsersSubscriber
    ...
    this.usersRepository
      ...
      .then(user => {
        if (user) {
          ...
          console.error('Usuário não encontrado durante a inserção.');
        }
      })
      .catch(error => {
        console.error('Erro ao buscar o usuário durante a inserção:', error);
      });
  }
  ...
  async beforeUpdate(event: UpdateEvent<User>) {
    ...
    try {
      ...
      if (user) {
        ...
        console.error('Usuário não encontrado durante a atualização.');
      }
    } catch (error) {
      console.error('Erro ao buscar o usuário durante a atualização:', error);
    }
  }
  ...
  async beforeRemove(event: RemoveEvent<User>) {
    ...
    try {
      ...
      if (user) {
        ...
        console.error('Usuário não encontrado durante a remoção.');
      }
    } catch (error) {
      console.error('Erro ao buscar o usuário durante a remoção:', error);
    }
  }
}
